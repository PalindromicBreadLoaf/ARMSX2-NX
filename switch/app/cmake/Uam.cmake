...
# deko3d only accepts DKSH, so uam is compiled into the NRO and the tfx fragment shader is
# specialised per PSSelector and translated at runtime, similar to how Vulkan does it, just
# with fallback Ubershaders.

if(NOT EXISTS "${UAM_ROOT}/source/compiler_iface.cpp")
	message(FATAL_ERROR
		"uam sources not found at ${UAM_ROOT}.\n"
		"Clone it next to the ARMSX2 checkout (git clone https://github.com/devkitPro/uam), point "
		"UAM_ROOT at an existing one, or configure with -DDK3D_RUNTIME_SHADERS=OFF to build with "
		"only the prebuilt tfx variants.")
endif()
...
foreach(_tool CMAKE_LINKER CMAKE_OBJCOPY)
	if(NOT ${_tool})
		message(FATAL_ERROR "${_tool} is not set. The uam blob cannot be isolated without it.")
	endif()
endforeach()
...
set(_uam_dir "${CMAKE_SOURCE_DIR}/pcsx2/GS/Renderers/DK3D/uam")
set(_uam_bridge "${_uam_dir}/UamBridge.cpp")
set(_uam_stderr "${_uam_dir}/UamStderr.h")
...
target_compile_definitions(uam_objects PRIVATE
	...
	DESKTOP
	_USE_MATH_DEFINES
	# uam's meson.build sets both on non-Windows. _GNU_SOURCE is what makes newlib advertise
	# _ISOC11_SOURCE, without which mesa's os_memory_stdc.h reaches for a header uam never imported.
	_GNU_SOURCE
	HAVE_POSIX_MEMALIGN
...
# mesa's C++ drags in libstdc++ template instantiations that PCSX2 also instantiates, and those
# arrive in COMDAT groups. Localising a group's symbols does not stop the group from winning the
# duplicate-elimination against PCSX2's own copy, which would leave PCSX2 calling a symbol that is
# now local to this blob, so the groups are flattened first.
add_custom_command(
