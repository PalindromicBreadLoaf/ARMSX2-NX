...
namespace
{
	...
	void ThrottledError(const char* format, ...) __attribute__((format(printf, 1, 2)));
	void ThrottledError(const char* format, ...)
	{
		static std::atomic<u64> s_last_tick{0};
		constexpr u64 TICKS_PER_SECOND = 19200000ULL;
		const u64 now = armGetSystemTick();
		const u64 last = s_last_tick.load(std::memory_order_relaxed);
		if (now - last < TICKS_PER_SECOND)
			return;
		s_last_tick.store(now, std::memory_order_relaxed);
	...
	bool UnmapRangeLocked(size_t first, size_t count)
	{
		...
		for (size_t i = 0; i < count;)
		{
			...
			if (R_FAILED(result))
			{
				...
				ThrottledError("HorizonFastmem: svcUnmapProcessMemory(%#llx, %#llx, %#llx) "
							   "failed: %#010x. Keeping shadow entry for retry",
					static_cast<unsigned long long>(destination), static_cast<unsigned long long>(source),
					static_cast<unsigned long long>(run * PAGE_SIZE), static_cast<unsigned>(result));
				return false;
			}
	...
	bool SetAliasProtectionLocked(size_t arena_page, bool writable)
	{
		...
		if (R_FAILED(result))
		{
			ThrottledError("HorizonFastmem: write-protect unmap of arena page %#llx "
						   "failed: %#010x. keeping shadow entry for retry",
				static_cast<unsigned long long>(destination), static_cast<unsigned>(result));
			return false;
		}
