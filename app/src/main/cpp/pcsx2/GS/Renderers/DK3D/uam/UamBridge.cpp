...
namespace
{
	...
	// The GPU reads the shader program header immediately before the code.
	...
	// ~DekoCompiler only releases the GLSL program.
...
void* UamCompileGlsl(const char* glsl, int stage, size_t* out_size, char** out_log)
{
	...
	*out_log = nullptr;
	...
	// Capture mesa's diagnostics without replacing the standard stderr stream.
