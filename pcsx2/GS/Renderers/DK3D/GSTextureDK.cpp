...
std::unique_ptr<GSTextureDK> GSTextureDK::Create(DkDevice device, GSDeviceDK* device_dk, Usage usage, Format format,
	int width, int height, int levels)
{
	...
	auto tex = std::unique_ptr<GSTextureDK>(new GSTextureDK(
		device, device_dk, memblock, block_size, block_flags, usage, format, width, height, levels, dk_format, is_depth));
...
bool GSTextureDK::DoUpdate(const GSVector4i& r, const void* data, int pitch, int layer)
{
	if (m_is_depth || !data)
		return false;
...
void GSDownloadTextureDK::DoCopyFromTexture(
